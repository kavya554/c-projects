...
while(fread(&x,sizeof(x),1,files)==1) /*buffer: it specifies the pointer to the block of memory with a size of at least (size*count) bytes to store the objects.
...
void delete_info()
{
	...
	else
	{
		...
		while(fread(&x,sizeof(x),1,files)==1)
		{
			...
			if(strcmp(x.fullname,name)==0) // info found
                fl=1;
		}
	fclose(files);
	fclose(file_temp);
